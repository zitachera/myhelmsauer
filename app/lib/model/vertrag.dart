...
class Vertrag {
  ...
  bool get hasSchadenTemplate =>
      meldeTemplates.any((t) => t.id == MeldeTemplate.schadenId);

  MeldeTemplate get schadenTemplate =>
      meldeTemplates.firstWhere((t) => t.id == MeldeTemplate.schadenId);
}

enum VertragStatus { aktiv, antrag, storno, other }

class VertragDokument {
  final String endpoint;
  final String titel;

  VertragDokument({required this.endpoint, required this.titel});
...
class MeldeTemplate {
  ...
  MeldeTemplate({required this.id, required this.name, required this.felder});
...
VertragStatus _jsonToVertragStatus(String status) =>
    VertragStatus.values.firstWhere(
      (v) => _vertragStatusToJson(v) == status,
...
String _meldeFeldKindToJson(MeldeFeldKind kind) =>
